...
@dataclass
class Ladder:
    """Two exclusion sets (plus V0 = raw), derived from the all-snapshot
    aggregate. Each set is the UNION of a degree criterion and a strength
    criterion — the two populations overlap only partially (~60% in the
    2024–2025 PKG aggregate), so a single-criterion exclusion misses a
    material share of dominant nodes.

      V1 'de-hubbed' : degree > P99.9  OR strength > P99.9
      V2 'mega-only' : degree > P99.99 OR strength > P99.99
    """
    v1_dehubbed: set = field(default_factory=set)
    v2_mega: set = field(default_factory=set)
    thresholds: dict = field(default_factory=dict)

    def exclusions(self, version: str) -> set:
        return {"V0": set(), "V1": self.v1_dehubbed,
                "V2": self.v2_mega}[version]
...
def build_ladder(snapshot_paths: list[str],
    ...
    derive union exclusion sets at two severities. Streams file-by-file to
    keep memory flat. Logs the degree/strength overlap so the data-driven
    choice of a union criterion is auditable."""
    deg_parts, str_parts = [], []
    ...
    def union_at(pct):
        ...
        return dset | sset, dset, sset, td, ts

    u1, d1, s1, td1, ts1 = union_at(p_broad)
    u2, d2, s2, td2, ts2 = union_at(p_strict)
    lad = Ladder(
        v1_dehubbed=u1, v2_mega=u2,
        thresholds={"degree_p999": float(td1), "strength_p999": float(ts1),
                    "degree_p9999": float(td2), "strength_p9999": float(ts2)},
    )
    log.info("ladder V1 de-hubbed: |deg|=%d |str|=%d overlap=%d union=%d",
             len(d1), len(s1), len(d1 & s1), len(u1))
    log.info("ladder V2 mega-only: |deg|=%d |str|=%d overlap=%d union=%d",
             len(d2), len(s2), len(d2 & s2), len(u2))
