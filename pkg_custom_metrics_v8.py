...
TROPHIC_TOL = 1e-6      # 1e-8 does not converge at 2M nodes in 2000 its
TROPHIC_MAXITER = 1000  # residual plateaus well before 2000


@safe("trophic", {"node": object, "trophic_level": float})
def trophic_levels(edges: pd.DataFrame, tol: float | None = None,
                   max_iter: int | None = None) -> pd.DataFrame:
    ...
    """
    tol = TROPHIC_TOL if tol is None else tol
    max_iter = TROPHIC_MAXITER if max_iter is None else max_iter
    nodes, A = _adj(edges, log_weight=True)
    ...
    Lc = L.tocsr()
    # Jacobi preconditioner: the Laplacian diagonal spans several orders
    # of magnitude at this scale, which is what stalls unpreconditioned CG
    dg = Lc.diagonal()
    Minv = sparse.diags(np.where(dg > 0, 1.0 / dg, 1.0))
    h, info = cg(Lc, v, rtol=tol, maxiter=max_iter, M=Minv)
    rel = float(np.linalg.norm(Lc @ h - v) / max(np.linalg.norm(v), 1e-30))
    if info > 0:
        log.warning("trophic CG did not converge (info=%d, n=%d, "
                    "rel_residual=%.2e, tol=%.0e) — using best iterate",
                    info, len(nodes), rel, tol)
    else:
        log.info("trophic CG converged (n=%d, rel_residual=%.2e)",
                 len(nodes), rel)
    h = h - h.min()
...
def participation_and_roles(edges: pd.DataFrame,
    ...
    sd_arr = np.nan_to_num(sd)
    z = np.divide(k_own.to_numpy() - mu, sd_arr,
                  out=np.zeros(len(sd_arr), dtype=float),
                  where=sd_arr > 0)
...
class Ladder:
    ...
    Default tiers:  P99  |  P99_9
    (exclude nodes with aggregate degree OR strength above that pctile)

    P99_99 was dropped after the first production run: it cost ~10 min per
    month and never carried an analytical decision. Pass percentiles=
    (99.0, 99.9, 99.99) explicitly if a sensitivity check needs it.
    """
...
def load_ladder(out_dir: str) -> Ladder:
    """Rebuild a Ladder from ladder_thresholds.csv + ladder_exclusions.csv.

    build_ladder() streams every snapshot to recompute aggregate degree and
    strength percentiles — several minutes of the session for a result that
    does not change between runs. When both CSVs are present we reload
    instead. Falls back to the caller if either is missing.

    ladder_exclusions.csv layout (as written by pkg_pipeline):
        node        — the BROADEST tier's exclusion set (lowest percentile)
        in_<tier>   — bool flag per stricter tier
    """
    import os as _os
    th_path = _os.path.join(out_dir, "ladder_thresholds.csv")
    ex_path = _os.path.join(out_dir, "ladder_exclusions.csv")
    if not (_os.path.exists(th_path) and _os.path.exists(ex_path)):
        raise FileNotFoundError(
            f"ladder CSVs not found in {out_dir}; run build_ladder() once")

    th = pd.read_csv(th_path).iloc[0].to_dict()
    reg = pd.read_csv(ex_path, dtype={"node": str})
    reg["node"] = reg["node"].astype(str)

    flag_cols = [c for c in reg.columns if c.startswith("in_")]
    # tier order is recoverable from the threshold keys, which were written
    # in ascending-percentile order by build_ladder
    tiers = []
    for k in th:
        if k.startswith("degree_"):
            t = k[len("degree_"):]
            if t not in tiers:
                tiers.append(t)
    if not tiers:
        tiers = [c[3:] for c in flag_cols]

    lad = Ladder()
    broad = tiers[0]
    lad.exclusion_sets[broad] = set(reg["node"])
    for c in flag_cols:
        t = c[3:]
        f = reg[c]
        if f.dtype == object:
            f = f.astype(str).str.strip().str.lower().isin(
                ("true", "1", "t", "yes"))
        lad.exclusion_sets[t] = set(reg.loc[f.astype(bool), "node"])
    lad.thresholds = {k: float(v) for k, v in th.items()
                      if isinstance(v, (int, float, np.floating))}

    log.info("ladder loaded from disk: %s",
             {t: len(s) for t, s in lad.exclusion_sets.items()})
    log.info("thresholds=%s", lad.thresholds)
    return lad
...
def build_node_typing(snapshot_paths: list[str],
    ...
    """DEPRECATED as of v6 — use pkg_customers.load_customers().typing.

    This typer reads name/NAICS off the snapshot edge files and has no
    access to `party_type`, so its entity_type is a pure name inference.
    It is retained only for reprocessing archived snapshots that still
    carry the old wide schema; anything it produces disagrees with the
    customer dimension on person-named organisations (trusts, estates,
    single-member LLCs) and must not be mixed with v6 outputs in the same
    table.
...
def typing_from_customers(customers) -> pd.DataFrame:
    """Adapter: pkg_customers.Customers -> the typing frame that
    composition_metrics() expects. Node ids stay strings on both sides."""
    cols = ["node", "entity_type", "naics_status", "node_type", "naics_clean"]
    t = customers.typing[cols]
    # v7 did .copy() then .astype(str) on `node` unconditionally — on a 30M
    # string column that materialises an object array of 30M Python strs for
    # no gain. Cast only if the ids are not already text.
    if not (pd.api.types.is_string_dtype(t["node"])
            or t["node"].dtype == object):
        t = t.assign(node=t["node"].astype(str))
    return t
...
def composition_metrics(edges: pd.DataFrame, typing: pd.DataFrame,
    ...
    # SCHEMA GUARD: share_* columns are emitted only for categories in
    # _CAT_SHORT, but the denominator sums ALL categories present. A new
    # node_type value therefore does not raise — it silently makes the
    # shares stop summing to 1.0, which looks like a plausible result.
    # (Live risk: PARTY_TYPE_WINS=False, or any finer entity taxonomy
    # being wired into node_type instead of entity_class.)
    unknown_cats = set(pd.unique(ntype.dropna())) - set(_CAT_SHORT)
    if unknown_cats:
        raise ValueError(
            f"node_type carries categories absent from _CAT_SHORT: "
            f"{sorted(unknown_cats)} — share_* columns would no longer sum "
            f"to 1.0. Add them to _CAT_SHORT (schema change to the node "
            f"table) or keep the finer taxonomy in `entity_class`.")
    ...
    for d, node_col, cp_col in (("in", "dest", "source"),
        ...
        bt = np.asarray(biz_tot, dtype=float)
        vl = np.asarray(valid, dtype=float)
        out[f"naics_coverage_biz_{d}"] = np.divide(
            vl, bt, out=np.full(bt.shape, np.nan), where=bt > 0)
