...
| Field | Values | Rule |
|---|---|---|
| `naics_status` | valid / placeholder / missing | Placeholder set is a **configurable list** (`PLACEHOLDER_NAICS`, extended from production EDA: {-1, -1\|UNKNOWN, ******, ******\|UNKNOWN, UNKNOWN, UNKNOWN\|UNKNOWN, ''}; production NAICS is a composite `code\|description` string — `naics_clean` keeps only the leading digit code) — enumerate via EDA on distinct NAICS values before production and extend; per-run match counts are logged as a sanity check |
...
| `node_type` | business_naics_valid / business_naics_placeholder / business_naics_missing / individual / unknown | Precedence **(revised after production EDA, 2026-07)**: valid NAICS wins; otherwise the name-based entity_type decides. The spec's original "placeholder implies business onboarding" rule was falsified in production — the NAICS field is never null there; non-enriched accounts *including individuals* carry `******\|UNKNOWN` (1.55M in one snapshot), so a populated placeholder signals nothing about being a business (82% of nodes were person-named with placeholder NAICS and would all have been mistyped). Root cause: the snapshot extraction itself injects the placeholder — `coalesce(c.naics, '******|UNKNOWN')` in the Cypher export — so source-null and source-placeholder are conflated and `business_naics_missing` is empty **by construction** unless the coalesce is removed from the extraction query |
...
**Implementation robustness (added after production incident, 2026-07)**: the typing join force-normalizes node IDs to *string* on both sides (typing table, edge list, hub set) — the typing table is built with `dtype=str` while edge lists parse numeric mdmIds as int64, and an unmatched dtype made the reindex return all-NaN, silently typing **every** counterparty as `unknown` (shares looked plausible: unknown ≈ 0.7, hub ≈ 0.3, everything else exactly 0). A **fail-fast guard** now samples 100K edge nodes and raises if the typing join misses >50%, so a key mismatch can never again produce a plausible-looking wrong table. **QA convention**: run a one-cell `.describe()` over each new metric family and eyeball it before loading to Hive — this incident was caught exactly that way.

**Schema-stability rule**: columns are never pruned dynamically, even when all-zero or mostly-NaN in a given month — per-month schema drift breaks the Hive tables and cross-month concat. Legitimately sparse columns (e.g. `naics2_entropy_*`, mostly NaN under the ≥5-counterparty guard) carry information in their NaNs; the known-constant `share_*_*_biz_missing` (zero by construction under the extraction coalesce) is kept deliberately and starts working the day the coalesce is removed.
