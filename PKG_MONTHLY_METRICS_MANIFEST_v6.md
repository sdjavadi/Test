...
>    discard the row holding the only valid coordinate (§0.1).
>
...
| `../data/customers.csv` | `mdm_id, customer_name, party_type, naics_cd, naics_desc, addr_loc_rec_type, longitude_degree, latitude_degree, zip_cd, state_or_province, city` *(optional: `household_id`, `customer_start_dt`)* | **Current state.** Not one row per customer — see §0.1 |
...
Absence is usually an empty string — but **the `******` / `UNKNOWN` sentinel is
present in the source table**, on person rows in particular. v3 asserted that
this token was a Cypher-extraction artefact and that a residual match count of
≈0 was expected; direct inspection of the source disproves that.
`PLACEHOLDER_NAICS` is therefore a live classifier, not a regression tripwire.
It is counted at every load and warns only above `PLACEHOLDER_WARN_SHARE`
(5% of rows), because a routine non-zero count is no longer news.

### 0.1 The customer file is an outer join — **new in v4**

`customers.csv` is produced by an **outer join between the MDM address table
and the customer table**. Three consequences, none of them cosmetic:
...
- **Identity fields** (`customer_name`, `party_type`, `naics_cd`, `naics_desc`,
  `household_id`, `customer_start_dt`) are **coalesced** across the group —
...
**Extraction SQL** — `party_type` must be in the pull:

```sql
SELECT c.mdm_id,
       c.customer_name,
       c.party_type,                 -- P = person, O = organisation
       c.naics_cd,
       c.naics_desc,
       c.household_id,
       c.customer_start_dt,
...
`date_of_birth_inc` is present in the source and is **deliberately not
ingested.** Age is a prohibited basis under ECOA / Regulation B, and placing it
...
`household_id`, `customer_start_dt`, `addr_loc_rec_type`, `lat`, `lon`, `zip5`,
...
Nodes absent from `customers.csv` are emitted with NA attributes,
...
proprietorships). v3 counted these as households. The headline number to
...
|---|---|---|---|
| **Customer dimension** — identity, NAICS hierarchy, typing, geo status, outer-join resolution | **`pkg_customers.py`** | — | v3: one pass over `customers.csv`, replacing two full passes over 23 snapshots. **v4: `party_type` typing, applicability axis, deterministic duplicate resolution, QA artefacts (`write_qa`)** |
...
| Counterparty composition, coverage, guarded sector mix | `pkg_custom_metrics.py` | raw | typing from the customer dimension; >50%-miss fail-fast guard retained; **v4 adds the `_CAT_SHORT` schema guard**. `build_node_typing()` (snapshot-based) is **deprecated** — no access to `party_type`, must not be mixed with v4 outputs |
