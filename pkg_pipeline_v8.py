...
DATA_DIR = "../data"
OUT_DIR = "../metrics"
CUSTOMERS_PATH = None      # None -> "<data_dir>/customers.parquet"
# Restrict the customer dimension to the graph node universe. Costs one
# streaming pass over the snapshots; cuts the rows entering the typing stage
# from ~30.8M MDM parties to the ~5.3M the graph actually touches. Off by
# default: it makes coverage against MDM_ALL underivable from the dimension.
# Turn on if the lean loader still does not fit the batch step.
RESTRICT_DIM_TO_GRAPH = False
CUSTOMERS_CSV = None       # deprecated alias, honoured if set
...
# the customer extract, so name+naics columns are never parsed: less IO, less RAM.
...
# the customer extract lands here so downstream apps need no dimension join.
IDENTITY_COLS = [
    "node", "cust_name", "party_type", "naics_cd", "naics_desc",
    "addr_loc_rec_type",
    "lat", "lon", "zip5", "zip3", "state", "city", "geo_status",
    "attr_profile", "shared_structure",
    # typing, merged onto attrs in run() so the node table is self-contained.
    # entity_type_observed/_inferred stay as separate columns permanently —
    # a declared attribute and a name heuristic are different evidence and
    # collapsing them makes the disagreement unauditable after the fact.
    "entity_type", "entity_type_observed", "entity_type_inferred",
    "entity_type_source", "entity_class",
    "naics_status", "naics_applicable", "naics_coverage_class", "node_type",
]

# Typing columns merged onto the customer dimension in run(). node_type_v5
# is carried too so the composition churn is reproducible from the parquet
# without re-reading the customer extract.
TYPING_COLS = [
    "node", "entity_type", "entity_type_observed", "entity_type_inferred",
    "entity_type_source", "entity_class", "naics_status", "naics_applicable",
    "naics_coverage_class", "node_type", "node_type_v5",
]
...
def node_identity(edges: pd.DataFrame,
    ...
    the customer extract are emitted with NA attributes and geo_status
    ...
    if len(missing):
        ...
        if "attr_profile" in cols:
            pad["attr_profile"] = "not_in_customers"
...
def run(data_dir: str = DATA_DIR, out_dir: str = OUT_DIR,
    ...
    with rt.step("setup", "-", "load_customers"):
        cpath = (customers_path or CUSTOMERS_PATH or CUSTOMERS_CSV
                 or os.path.join(data_dir, "customers.parquet"))
        nfilter = None
        if RESTRICT_DIM_TO_GRAPH:
            nfilter = pc.node_universe(os.path.join(data_dir, "cust_*.csv"))
        customers = pc.load_customers(cpath, node_filter=nfilter)
        del nfilter
    coords = customers.coords
    # ONE frame. v7 held attrs (30M) + typing (30M) + this merge's result
    # (30M) simultaneously for a single dimension; pkg_customers now returns
    # the typing columns inside attrs and exposes .typing as a projection.
    attrs = customers.attrs
    typing = cm.typing_from_customers(customers)
    typing.to_parquet(os.path.join(out_dir, "node_typing.parquet"),
                      index=False)
    # observed-vs-inferred confusion, node_type churn against the pre-
    # party_type rule, the outer-join profile and the source dtype report —
    # written every run, read before any composition share is briefed
    pc.write_qa(customers, out_dir)
    ...
    for path in paths:
        ...
        for version in versions:
            ...
            for c in ("node", "cust_name", "party_type", "naics_cd",
                      "naics_desc",
                      "addr_loc_rec_type", "zip5", "zip3", "state", "city",
                      "geo_status", "attr_profile", "geo_locality_class",
                      "community_id", "ga_role",
                      "entity_type", "entity_type_observed",
                      "entity_type_inferred", "entity_type_source",
