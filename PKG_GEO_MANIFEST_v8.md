...
**Node type** (`node_type`, from `customers.parquet`) — `business_naics_valid` /
...
### Block A — Location (from `customers.parquet`, static)

Carried on every node row; also written once to `../metrics/customers_dim.parquet`.

| Column | Definition |
|---|---|
| `geo_status` | `valid` / `placeholder` / `missing` / `not_in_customers` — Categorical with declared categories |
...
The customer extract carries no address lines, so `addr_loc_rec_type` is the
only placeholder signal available. Name-based PO-box matching is **not** used — it
would flag businesses literally named "PO Box Trading Co".

**The extract is an outer join of the address and customer tables**, so a
...
> **⚠ ZIP dtype hazard (parquet).** If `zip_cd` is extracted as an INTEGER,
> leading zeros are destroyed: `02108` arrives as `2108`. The loader
> zero-pads and warns, but a genuinely short value is then indistinguishable
> from a 0-prefixed one. The affected population is **the entire 0-prefixed
> ZIP range — all of New England, NJ and PR** — and the failure is silent:
> `zip3` becomes `210` instead of `021`, which keys a different CBSA rather
> than no CBSA. `zip_cd` **must be CAST AS STRING in the extract**
> (`PKG_MONTHLY_METRICS_MANIFEST` §0.1); verify on
> `qa/customers_source_schema.csv` before any ZIP-keyed geographic result.
...
*Not yet in the pipeline* — requires address lines, which the customer extract does
...
| Block B placeholder registry into the pipeline | Needs address lines, absent from the customer extract |
...
| 1.4 | 2026-08 | Customer dimension made memory-lean after the v5 OOM (categorical codes, vectorised name typer, single frame). **Block A columns are now Categorical with declared category sets** — `geo_status` and `attr_profile` include `not_in_customers` so the pipeline's pad for graph nodes absent from the dimension is a declared value, not a silent NaN |
| 1.3 | 2026-08 | Customer dimension source moved to **parquet**; ZIP/id dtype hazards documented (leading-zero destruction, float id precision); `household_id` / `customer_start_dt` removed from Block A |
