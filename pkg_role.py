...
def assign_flow_role(df: pd.DataFrame) -> np.ndarray:
    ...
    med_recip = _q(df.get("reciprocity_node_w", pd.Series(dtype=float)), 0.5)
...
def assign_dependence_role(df: pd.DataFrame) -> np.ndarray:
    med_hin = _q(df["hhi_in"], 0.5)
    med_hout = _q(df["hhi_out"], 0.5)
    ...
    return np.select(
        ...
        ["infra_dependent", "anchor_dependent", "captive_payer",
         "diversified"],
        default="moderate")


def assign_embeddedness_role(df: pd.DataFrame) -> np.ndarray:
    thr_core = _q(df.get("core_number", pd.Series(dtype=float)), 0.90)
    lo_core = _q(df.get("core_number", pd.Series(dtype=float)), 0.25)
    ...
    return np.select(
        ...
        ["connector", "local_anchor", "embedded_local", "peripheral"],
        default="intermediate")


def assign_dynamics_role(df: pd.DataFrame) -> np.ndarray:
    ...
    return np.select(
        [
            tenure <= 2,
            gap > 1,
            (lost >= 0.4) | ((mom <= -0.3) & (pjv <= p25)),
            (new >= 0.4) & (mom > 0.1),
            (pjv >= p50) & (np.abs(mom) < 0.2),
        ],
...
def run(node_dir: str = NODE_DIR, out_dir: str = OUT_DIR):
    ...
    for path in paths:
        ...
        for version, df in panel.groupby("version", observed=True):
            ...
            roles["dynamics_role"] = assign_dynamics_role(df)
