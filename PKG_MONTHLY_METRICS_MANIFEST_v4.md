...
**Resumable.** A month whose node file exists is skipped. Cross-month tracker
...
P99_99 is **removed** in v3 — a fourth version's compute for a tier that never
carried an analytical decision.
...
Measured January 2024 baseline (V0: 2.72M edges / 2.07M nodes): **~12.2 min per
month** across three versions, ≈4.7 h for 23 months. **Trophic was 37% of V0
runtime** and is the first optimisation target if the budget binds.

Savings in v3: no ladder rebuild (23 CSV reads), no typing pass over snapshots,
and a 4-column snapshot read instead of 8.
