...
def gpu_clustering(edges: pd.DataFrame) -> pd.DataFrame:
    ...
    if HAS_GPU:
        ...
        tri["clustering_coef"] = np.where(
            tri["k"] >= 2, 2.0 * tri["tri"] / (tri["k"] * (tri["k"] - 1)),
            np.nan)
    ...
    cc = np.where(k >= 2, 2.0 * tri / (k * (k - 1)), np.nan)
...
def node_identity(edges: pd.DataFrame) -> pd.DataFrame:
    """cust_name + naics_desc per node, from the snapshot itself, so the
    metrics/roles tables never need a join to the customer dimension.
    naics_desc = the text after '|' in the composite NAICS, only when the
    code part is valid (placeholder descriptions like 'UNKNOWN' -> NA)."""
    nodes = pd.concat([
        edges[["source", "source_name", "source_naics"]].rename(columns={
            "source": "node", "source_name": "cust_name",
            "source_naics": "naics_raw"}),
        edges[["dest", "dest_name", "dest_naics"]].rename(columns={
            "dest": "node", "dest_name": "cust_name",
            "dest_naics": "naics_raw"}),
    ]).drop_duplicates("node")
    nr = nodes["naics_raw"].astype("string")
    valid = nr.str.match(r"^\d{2}").fillna(False)
    desc = nr.str.split("|", n=1).str[1].str.strip()
    nodes["naics_desc"] = desc.where(valid)
    return nodes[["node", "cust_name", "naics_desc"]].reset_index(drop=True)
...
def node_metrics(edges: pd.DataFrame
    ...
    blocks = [
        node_identity(edges),
        hier,
        node_naics_homophily(edges, hier),
        ...
        gpu_clustering(edges),
        cm.node_reciprocity(edges).rename(
...
def run(data_dir: str = DATA_DIR, out_dir: str = OUT_DIR):
    ...
    for path in paths:
        ...
        for version in versions:
            ...
            for c in ("node", "cust_name", "naics_desc",
        ...
        del raw, hub_exp, comp, n_rows, g_rows, d_rows, c_rows
        gc.collect()
